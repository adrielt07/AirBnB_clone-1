#!/usr/bin/python3
from fabric.api import put, run, local, env, execute
import os
import time
...
def do_pack():
    ...
    executes = "tar -cvzf ./versions/web_static_{}.tgz web_static".format(
        timestr)
    result = local(executes)
    if result.succeeded:
...
def deploy():
    archive_location = execute(do_pack)
    if archive_location is None:
        return None
    print(archive_location)
    name = archive_location['35.227.25.29'] + '.tgz'
    print(name)
    res = execute(do_deploy(name))
    return (res)
